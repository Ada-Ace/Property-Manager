...
// Standardised date formatter â†?dd-MMM-yyyy (e.g. 19-Mar-2026)
...
// Shorthand: convert an ISO/date string â†?dd-MMM-yyyy, stripping time part first
const fmtDate = (str) => {
    if (!str) return 'â€?;
    try {
...
function ManagerDashboard({ tenants, propertyUnits, utilityBills, tasks, tenantMessages, currency = 'USD', onAddUnit, onAddTenant, onEditTenant, onUpdateFittings, onAddBill, onAddTask }) {
    ...
    // Currency tag helper â€?prepends ISO code as a label
                ...
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-10 pb-6 border-b border-white/5 gap-4">
                    <div>
                        ...
                        <p className="text-[10px] text-slate-500 font-black uppercase tracking-[0.2em] mt-2">
                            {currentMonthLabel} Â· Sorted by urgency
                        </p>
                    </div>
                    {soonDueCount > 0 && (
                                ...
                                <div className="flex flex-wrap items-center gap-4 md:gap-6 w-full md:w-auto">
                                    <div className="text-left md:text-right">
                                        ...
                                        <p className={`text-[9px] font-black uppercase mt-0.5 ${
                                            ...
                                            {rent.daysUntil === 0 ? 'ðŸ”´ Due Today' : isOverdue ? `${Math.abs(rent.daysUntil)}d overdue` : `${rent.daysUntil}d left`}
                                        </p>
                                    </div>
            ...
            <div className="premium-card rounded-[2.5rem] p-8">
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-10 pb-6 border-b border-white/5 gap-4">
                    <div>
                        ...
                        <p className="text-[10px] text-slate-500 font-black uppercase tracking-[0.2em] mt-2">Tenant inbox Â· All messages</p>
                    </div>
                ...
                <div className="premium-card rounded-[2.5rem] p-8">
                    <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-10 border-b border-white/5 pb-6 gap-4">
                        <div>
                            ...
                            <p className="text-[10px] text-slate-500 font-black uppercase tracking-[0.2em] mt-2">Bills per tenant Â· {new Date(effectiveMonth + '-01').toLocaleString('default', { month: 'long', year: 'numeric' })}</p>
                        </div>
                        <div className="flex items-center gap-3">
                                    ...
                                    <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-6 pb-6 border-b border-white/5">
                                        <div className="flex gap-4 items-center">
                                            ...
                                            <div>
                                                ...
                                                <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest mt-1"><span className="text-indigo-400">{fmtDate(bill.date)}</span> â€?{bill.mode === 'equal' ? 'Standard Split' : 'Designated Split'}</p>
                                            </div>
                                        </div>
                                        <div className="text-right">
                    ...
                    return (
                        <div key={task.id} className="bg-slate-900/50 rounded-3xl p-6 border border-white/5 flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
                            <div>
                                ...
                                <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest mt-1">Tenant: <span className="text-indigo-400">{tenant.name}</span> â€?Unit {tenant.unit}</p>
